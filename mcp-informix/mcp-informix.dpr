...
uses
  ...
  MCPTool.Informix in 'MCPTool.Informix.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-informix';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.Informix.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-informix] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-informix] Fatal: ' + E.Message); Halt(1); end;
  end;
end.
