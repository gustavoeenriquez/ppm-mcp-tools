...
uses
  ...
  MCPTool.Calculator in 'MCPTool.Calculator.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName        := 'mcp-calculator';
    MCPServer.ServerVersion := '1.0.12';
    MCPServer.Port              := Port;
    MCPServer.CorsEnabled       := True;
    MCPServer.CorsAllowedOrigins := '*';

    MCPTool.Calculator.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-calculator] SSE  -> http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-calculator] HTTP -> http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-calculator] Stdio -- waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-calculator] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
