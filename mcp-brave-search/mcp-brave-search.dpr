...
uses
  ...
  MCPTool.BraveSearch in 'MCPTool.BraveSearch.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName       := 'mcp-brave-search';
    MCPServer.ServerVersion := '1.1.6';
    MCPServer.Port             := Port;
    MCPServer.CorsEnabled      := True;
    MCPServer.CorsAllowedOrigins := '*';

    MCPTool.BraveSearch.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-brave-search] SSE  --> http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-brave-search] HTTP --> http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-brave-search] Stdio -- waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-brave-search] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
