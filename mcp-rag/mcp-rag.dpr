...
uses
  ...
  MCPTool.RAG in 'MCPTool.RAG.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-rag';
    MCPServer.ServerVersion := '1.1.6';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.RAG.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-rag] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-rag] Fatal: ' + E.Message); Halt(1); end;
  end;
end.
