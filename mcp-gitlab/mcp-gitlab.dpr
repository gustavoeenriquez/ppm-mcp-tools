...
uses
  ...
  MCPTool.GitLab in 'MCPTool.GitLab.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-gitlab';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.GitLab.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-gitlab] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-gitlab] Fatal: ' + E.Message); Halt(1); end;
  end;
end.