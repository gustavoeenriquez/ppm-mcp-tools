...
uses
  ...
  MCPTool.Zoom in 'MCPTool.Zoom.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-zoom';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.Zoom.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-zoom] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-zoom] Fatal: ' + E.Message); Halt(1); end;
  end;
end.