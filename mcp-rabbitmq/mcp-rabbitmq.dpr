...
uses
  ...
  MCPTool.RabbitMQ in 'MCPTool.RabbitMQ.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName         := 'mcp-rabbitmq';
    MCPServer.Port               := Port;
    MCPServer.CorsEnabled        := True;
    MCPServer.CorsAllowedOrigins := '*';

    MCPTool.RabbitMQ.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-rabbitmq] SSE  -> http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-rabbitmq] HTTP -> http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-rabbitmq] Stdio -- waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-rabbitmq] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
