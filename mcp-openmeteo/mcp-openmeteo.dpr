...
uses
  ...
  MCPTool.OpenMeteo in 'MCPTool.OpenMeteo.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName         := 'mcp-openmeteo';
    MCPServer.ServerVersion := '1.0.4';
    MCPServer.Port               := Port;
    MCPServer.CorsEnabled        := True;
    MCPServer.CorsAllowedOrigins := '*';

    MCPTool.OpenMeteo.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-openmeteo] SSE  -> http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-openmeteo] HTTP -> http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-openmeteo] Stdio -- waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-openmeteo] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
