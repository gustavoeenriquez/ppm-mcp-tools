...
uses
  ...
  MCPTool.Extract in 'MCPTool.Extract.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName        := 'mcp-extract';
    MCPServer.ServerVersion := '1.2.8';
    MCPServer.Port              := Port;
    MCPServer.CorsEnabled       := True;
    MCPServer.CorsAllowedOrigins := '*';

    MCPTool.Extract.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-extract] SSE  → http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-extract] HTTP → http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-extract] Stdio — waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-extract] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
