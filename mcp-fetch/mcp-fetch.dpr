...
uses
  ...
  MCPTool.Fetch in 'MCPTool.Fetch.pas';
...
begin
  ...
  try
    ...
    MCPTool.Fetch.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-fetch] SSE  -> http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-fetch] HTTP -> http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-fetch] Stdio -- waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-fetch] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
