...
uses
  ...
  MCPTool.Chroma in 'MCPTool.Chroma.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-chroma';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.Chroma.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-chroma] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-chroma] Fatal: ' + E.Message); Halt(1); end;
  end;
end.
