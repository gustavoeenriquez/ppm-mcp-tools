  ...
  "examples": [
    {
      ...
      "input": { "tool": "conta_puc", "operation": "GetPlanCuentas", "params": "{}" },
      "output": [ { "codigo": "110505", "nombre": "Caja general" } ]
    },
    {
      "description": "Vouchers of a date range",
      "input": { "tool": "conta_comprobantes", "operation": "GetComprobantes", "params": "{\"fecha_desde\":\"2026-01-01\",\"fecha_hasta\":\"2026-12-31\"}" },
      "output": [ { "tipo_codigo": "FV", "numero": 14, "estado": "CONTABILIZADO" } ]
    },
    {
      "description": "A write operation is rejected client-side",
      "input": { "tool": "conta_puc", "operation": "SaveCuenta", "params": "{}" },
      "output": { "ok": false, "error": "\"SaveCuenta\" modifica datos y este servidor es de solo lectura." }
    }
  ],
  "tags": ["contabilidad","accounting","colombia","erp","reportes","consultas","readonly","query","safe","agent","datasnap"],
