...
uses
  ...
  MCPTool.Regex in 'MCPTool.Regex.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName        := 'mcp-regex';
    MCPServer.ServerVersion := '1.0.14';
    MCPServer.Port              := Port;
    MCPServer.CorsEnabled       := True;
    MCPServer.CorsAllowedOrigins := '*';

    MCPTool.Regex.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-regex] SSE  -> http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-regex] HTTP -> http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-regex] Stdio -- waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-regex] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
