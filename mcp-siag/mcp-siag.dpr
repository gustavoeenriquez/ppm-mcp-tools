...
uses
  ...
  MCPTool.Siag in 'MCPTool.Siag.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName         := 'mcp-siag';
    MCPServer.ServerVersion := '1.0.2';
    MCPServer.Port               := Port;
    MCPServer.CorsEnabled        := True;
    MCPServer.CorsAllowedOrigins := '*';

    MCPTool.Siag.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-siag] SSE  -> http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-siag] HTTP -> http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-siag] Stdio -- waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-siag] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
