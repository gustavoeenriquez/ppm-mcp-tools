...
uses
  ...
  MCPTool.Qdrant in 'MCPTool.Qdrant.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-qdrant';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.Qdrant.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-qdrant] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-qdrant] Fatal: ' + E.Message); Halt(1); end;
  end;
end.
