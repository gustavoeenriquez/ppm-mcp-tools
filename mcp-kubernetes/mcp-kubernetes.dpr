...
uses
  ...
  MCPTool.Kubernetes in 'MCPTool.Kubernetes.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName         := 'mcp-kubernetes';
    MCPServer.Port               := Port;
    MCPServer.CorsEnabled        := True;
    MCPServer.CorsAllowedOrigins := '*';

    MCPTool.Kubernetes.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-kubernetes] SSE  -> http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-kubernetes] HTTP -> http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-kubernetes] Stdio -- waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-kubernetes] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
