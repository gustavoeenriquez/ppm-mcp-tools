...
uses
  ...
  MCPTool.Wikipedia in 'MCPTool.Wikipedia.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName         := 'mcp-wikipedia';
    MCPServer.Port               := Port;
    MCPServer.CorsEnabled        := True;
    MCPServer.CorsAllowedOrigins := '*';

    MCPTool.Wikipedia.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-wikipedia] SSE  -> http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-wikipedia] HTTP -> http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-wikipedia] Stdio -- waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-wikipedia] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
