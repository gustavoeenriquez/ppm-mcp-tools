...
uses
  ...
  MCPTool.WebCrawl in 'MCPTool.WebCrawl.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName        := 'mcp-webcrawl';
    MCPServer.Port              := Port;
    MCPServer.CorsEnabled       := True;
    MCPServer.CorsAllowedOrigins := '*';

    MCPTool.WebCrawl.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-webcrawl] SSE  → http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-webcrawl] HTTP → http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-webcrawl] Stdio — waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-webcrawl] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
