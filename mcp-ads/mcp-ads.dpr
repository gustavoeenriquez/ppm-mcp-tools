...
uses
  ...
  MCPTool.ADS in 'MCPTool.ADS.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-ads';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.ADS.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-ads] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-ads] Fatal: ' + E.Message); Halt(1); end;
  end;
end.
