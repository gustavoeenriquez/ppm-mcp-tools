...
uses
  ...
  MCPTool.Salesforce in 'MCPTool.Salesforce.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-salesforce';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.Salesforce.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-salesforce] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-salesforce] Fatal: ' + E.Message); Halt(1); end;
  end;
end.