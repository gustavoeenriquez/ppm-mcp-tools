...
uses
  ...
  MCPTool.Linear in 'MCPTool.Linear.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-linear';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.Linear.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-linear] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-linear] Fatal: ' + E.Message); Halt(1); end;
  end;
end.