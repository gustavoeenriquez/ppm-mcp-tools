...
uses
  ...
  MCPTool.Audio in 'MCPTool.Audio.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName         := 'mcp-audio';
    MCPServer.ServerVersion := '1.1.8';
    MCPServer.Port               := Port;
    MCPServer.CorsEnabled        := True;
    MCPServer.CorsAllowedOrigins := '*';

    MCPTool.Audio.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-audio] SSE  → http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-audio] HTTP → http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-audio] Stdio — waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-audio] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
