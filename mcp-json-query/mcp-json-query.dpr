...
uses
  ...
  MCPTool.JsonQuery in 'MCPTool.JsonQuery.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName        := 'mcp-json-query';
    MCPServer.ServerVersion := '1.1.10';
    MCPServer.Port              := Port;
    MCPServer.CorsEnabled       := True;
    MCPServer.CorsAllowedOrigins := '*';

    MCPTool.JsonQuery.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-json-query] SSE  -> http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-json-query] HTTP -> http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-json-query] Stdio -- waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-json-query] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
