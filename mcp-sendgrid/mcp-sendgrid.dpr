...
uses
  ...
  MCPTool.SendGrid in 'MCPTool.SendGrid.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-sendgrid';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.SendGrid.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-sendgrid] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-sendgrid] Fatal: ' + E.Message); Halt(1); end;
  end;
end.