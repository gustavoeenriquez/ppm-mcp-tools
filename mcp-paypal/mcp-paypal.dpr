...
uses
  ...
  MCPTool.Paypal in 'MCPTool.Paypal.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-paypal';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.Paypal.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-paypal] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-paypal] Fatal: ' + E.Message); Halt(1); end;
  end;
end.