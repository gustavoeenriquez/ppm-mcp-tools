...
uses
  ...
  MCPTool.TextTransform in 'MCPTool.TextTransform.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName        := 'mcp-text-transform';
    MCPServer.ServerVersion := '1.0.16';
    MCPServer.Port              := Port;
    MCPServer.CorsEnabled       := True;
    MCPServer.CorsAllowedOrigins := '*';

    MCPTool.TextTransform.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-text-transform] SSE  -> http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-text-transform] HTTP -> http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-text-transform] Stdio -- waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-text-transform] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
