...
uses
  ...
  MCPTool.DynamoDB in 'MCPTool.DynamoDB.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-dynamodb';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.DynamoDB.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-dynamodb] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-dynamodb] Fatal: ' + E.Message); Halt(1); end;
  end;
end.