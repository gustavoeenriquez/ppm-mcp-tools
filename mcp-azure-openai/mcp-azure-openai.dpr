...
uses
  ...
  MCPTool.AzureOpenAI in 'MCPTool.AzureOpenAI.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-azure-openai';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.AzureOpenAI.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-azure-openai] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-azure-openai] Fatal: ' + E.Message); Halt(1); end;
  end;
end.