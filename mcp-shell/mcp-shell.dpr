...
uses
  ...
  MCPTool.Shell in 'MCPTool.Shell.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName       := 'mcp-shell';
    MCPServer.ServerVersion := '1.1.9';
    MCPServer.Port             := Port;
    MCPServer.CorsEnabled      := True;
    MCPServer.CorsAllowedOrigins := '*';

    MCPTool.Shell.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-shell] SSE  → http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-shell] HTTP → http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-shell] Stdio — waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-shell] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
