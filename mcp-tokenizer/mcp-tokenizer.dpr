...
uses
  ...
  MCPTool.Tokenizer in 'MCPTool.Tokenizer.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName        := 'mcp-tokenizer';
    MCPServer.ServerVersion := '1.0.14';
    MCPServer.Port              := Port;
    MCPServer.CorsEnabled       := True;
    MCPServer.CorsAllowedOrigins := '*';

    MCPTool.Tokenizer.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-tokenizer] SSE  -> http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-tokenizer] HTTP -> http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-tokenizer] Stdio -- waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-tokenizer] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
