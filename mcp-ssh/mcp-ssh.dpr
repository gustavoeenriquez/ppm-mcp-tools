...
uses
  ...
  MCPTool.SSH in 'MCPTool.SSH.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-ssh';
    MCPServer.ServerVersion := '1.2.10';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.SSH.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-ssh] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-ssh] Fatal: ' + E.Message); Halt(1); end;
  end;
end.
