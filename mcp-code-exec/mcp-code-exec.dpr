...
uses
  ...
  MCPTool.CodeExec in 'MCPTool.CodeExec.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-code-exec';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.CodeExec.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-code-exec] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-code-exec] Fatal: ' + E.Message); Halt(1); end;
  end;
end.
