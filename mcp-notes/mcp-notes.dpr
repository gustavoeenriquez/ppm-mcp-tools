...
uses
  ...
  MCPTool.Notes in 'MCPTool.Notes.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-notes';
    MCPServer.ServerVersion := '1.0.10';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.Notes.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-notes] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-notes] Fatal: ' + E.Message); Halt(1); end;
  end;
end.
