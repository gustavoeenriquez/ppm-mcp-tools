program mcp_sequential_thinking;
...
uses
  ...
  MCPTool.SequentialThinking in 'MCPTool.SequentialThinking.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-sequential-thinking';
    MCPServer.ServerVersion := '1.1.6';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.SequentialThinking.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-sequential-thinking] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-sequential-thinking] Fatal: ' + E.Message); Halt(1); end;
  end;
end.
