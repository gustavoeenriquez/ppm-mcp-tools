...
uses
  ...
  MCPTool.AlphaVantage in 'MCPTool.AlphaVantage.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName       := 'mcp-alphavantage';
    MCPServer.Port             := Port;
    MCPServer.CorsEnabled      := True;
    MCPServer.CorsAllowedOrigins := '*';

    MCPTool.AlphaVantage.RegisterTools(MCPServer);
    MCPServer.Start;

    if MCPServer is TAiMCPSSEHttpServer then
      WriteLn(ErrOutput, Format('[mcp-alphavantage] SSE  --> http://localhost:%d/sse', [Port]))
    else if MCPServer is TAiMCPHttpServer then
      WriteLn(ErrOutput, Format('[mcp-alphavantage] HTTP --> http://localhost:%d/mcp', [Port]))
    else
      WriteLn(ErrOutput, '[mcp-alphavantage] Stdio -- waiting for JSON-RPC on stdin.');
    ...
    on E: Exception do
    begin
      WriteLn(ErrOutput, '[mcp-alphavantage] Fatal: ' + E.Message);
      Halt(1);
    end;
  end;
