...
uses
  ...
  MCPTool.MongoDB in 'MCPTool.MongoDB.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-mongodb';
    MCPServer.ServerVersion := '1.2.4';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.MongoDB.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-mongodb] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-mongodb] Fatal: ' + E.Message); Halt(1); end;
  end;
end.
