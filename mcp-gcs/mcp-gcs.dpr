...
uses
  ...
  MCPTool.GCS in 'MCPTool.GCS.pas';
...
begin
  ...
  try
    ...
    MCPServer.ServerName := 'mcp-gcs';
    MCPServer.Port := Port;
    MCPServer.CorsEnabled := True;
    MCPServer.CorsAllowedOrigins := '*';
    MCPTool.GCS.RegisterTools(MCPServer);
    MCPServer.Start;
    WriteLn(ErrOutput, '[mcp-gcs] ready.');
    while True do Sleep(1000);
  except
    on E: Exception do begin WriteLn(ErrOutput, '[mcp-gcs] Fatal: ' + E.Message); Halt(1); end;
  end;
end.